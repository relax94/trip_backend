...
export class NotificationManager {
    ...
    saveNotify(creator_id, sender_id, type, text, core_id, fn) {
        new Notification({
    ...
    postNotify(post) {
        if (post && post['owner_id']) {
            ...
            return this.saveNotify(creator, senderId, obj.method, notifyText, post['wall_id'], (err, resposnse) => {
                if (!err) {
    ...
    getAllNotification(criteria, fn) {
        return this.Store.findInstancesWithPopulate(Notification, criteria, { posted_by: -1 }, "sender", fn);
    }
