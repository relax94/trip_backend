...
export let GetTripSnapshot = {
    method: 'GET',
    path: '/trip/getsnapshot',
    handler: (req, reply) => {
        let event_id = req.query.event_id;
        if (event_id) {
            let criteria = { event_id };
            return TM.getTrip(criteria, (getResponse) => {
                if (getResponse && getResponse.r && getResponse.r.map_snapshot_url) {
                    let fileRoute = getResponse.r.map_snapshot_url;
                    return reply.redirect(fileRoute /*+ "?size=150"*/);
                }
                else
                    return reply({ state: false }).code(404);
            });
        }
        return reply({ state: false }).code(500);
    }
}